...
class ResponseParser {
    ...
    static validateControl(control) {
        return {
            ...
            frequency: this.mapFrequency(control.frequency) || 'Ad-hoc',
    ...
    static mapFrequency(frequency) {
        if (!frequency) return 'Ad-hoc';

        const frequencyMappings = {
            'every invoice': 'Per Transaction',
            'each invoice': 'Per Transaction',
            'per invoice': 'Per Transaction',
            'every transaction': 'Per Transaction',
            'each transaction': 'Per Transaction',
            'per transaction': 'Per Transaction',
            'each application': 'Per Event',
            'every application': 'Per Event',
            'per application': 'Per Event',
            'each request': 'Per Event',
            'every request': 'Per Event',
            'per request': 'Per Event',
            'when required': 'As Required',
        ...
        return frequencyMappings[normalizedFrequency] || 'Ad-hoc';
    }
