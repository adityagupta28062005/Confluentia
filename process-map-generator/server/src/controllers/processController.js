...
class ProcessController {
    async extractProcesses(req, res, next) {
        try {
            ...
            res.json({
                success: true,
                data: {
                    processId: savedProcesses[0]?._id || document._id, // Use first process ID or document ID
                    documentId: document._id,
    ...
    async getResults(req, res, next) {
        try {
            ...
            // Try to find by process ID first, then by document ID
            let process = await Process.findById(processId)
                .populate('risks')
                .populate('controls')
                .populate('document');

            if (!process) {
                // If not found by process ID, try to find by document ID
                const document = await Document.findById(processId).populate('processes');
                if (document && document.processes.length > 0) {
                    process = await Process.findById(document.processes[0])
                        .populate('risks')
                        .populate('controls')
                        .populate('document');
                }
            ...
            if (!process) {
                return res.status(404).json({
                    success: false,
                    error: 'Process or results not found'
                });
            }
            ...
            const processData = {
                ...
                activities: process.steps.map((step, index) => ({
                    id: `activity_${index + 1}`,
                    name: step.name || step.activity || step,
                    description: step.description || '',
                    type: step.type || 'UserTask',
                    performer: step.performer || step.actor || ''
                })),
                risks: process.risks.map((risk, index) => ({
                    id: `risk_${index + 1}`,
                    name: risk.name,
                ...
                controls: process.controls.map((control, index) => ({
                    id: `control_${index + 1}`,
                    name: control.name,
                    description: control.description,
                    type: control.type || 'Preventive',
                    frequency: control.frequency || 'As needed',
                    owner: control.owner || ''
                }))
            };
    ...
    // New method for target JSON format
    async getDocumentResults(req, res, next) {
        try {
            const { documentId } = req.params;

            // Find all processes for this document
            const processes = await Process.find({ document: documentId })
                .populate('risks')
                .populate('controls')
                .populate('document');
            ...
            // Format in target structure
            const result = {
                found_processes: processes.length > 0,
            ...
            logger.error('Get document results error:', error);
            next(error);
        }
    }

    // Get all processes for a document with BPMN XML
    async getProcessesForDocument(req, res, next) {
        try {
            const { documentId } = req.params;
            ...
            // Format each process for the frontend
            ...
            logger.error('Get processes for document error:', error);
            next(error);
        }
    }
}
